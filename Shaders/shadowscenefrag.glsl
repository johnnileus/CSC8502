...
void main(void) {
    ...
    vec3 shadowNDC = IN.shadowProj.xyz / IN.shadowProj.w; // Transform to normalized device coordinates (NDC)

    if (abs(shadowNDC.x) < 1.0f &&
        abs(shadowNDC.y) < 1.0f &&
        abs(shadowNDC.z) < 1.0f) {
        // Adjust shadow coordinates for texture sampling (bias)
        vec3 biasCoord = shadowNDC * 0.5f + 0.5f;

        // Sample the depth from the shadow map
        float shadowZ = texture(shadowTex, biasCoord.xy).x;

        // Compare the current fragment depth with the shadow map depth to determine shadowing
        if (shadowZ < biasCoord.z) {
            shadow = 0.0f; // Fragment is in shadow
        }
    }
